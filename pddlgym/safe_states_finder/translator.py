...
def find_reversible_events(sas_task: SASTask) -> Tuple[
    ...
    undecided_events: List[SASOperator] = set(events)
    irreversible_events: List[SASOperator] = set()
    reversible_events_with_states: List[Tuple[SASOperator, int, List]] = set()
    ...
    reversible_events: List[SASOperator] = set(e for e, v, s in reversible_events_with_states)

    for e, v, s in reversible_events_with_states:
        graph = eoDTG(v, sas_task.variables, reversible_events)
